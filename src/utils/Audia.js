...
let Audia = (function() {
  ...
  // Math helper
  var isTrue =  function(test, major, minor) {
    return Math.min (Math.max(+test, major), minor);
  }
  ...
  if( hasWebAudio ){
    ...
    // Load audio helper
    var buffersCache = {};
    var loadAudioFile = function(object, urlParam) {
      ...
      var onLoad = function(buffer) {
        ...
        object.dispatchEvent("canplay");
        object.dispatchEvent("canplaythrough");
    ...
    // play(startAtTime)
    Audia.prototype.play = function (startAtTime) {
      ...
      if( this.bufferSource && undefined !== this.bufferSource.noteOn){
        this.bufferSource.start = this.bufferSource.noteOn
      }
      let currentTime = audioContext.currentTime;
      this.bufferSource.start(startAtTime);
      //this.bufferSource.start(currentTime+5);
    };

    // pause()
    Audia.prototype.pause = function (stopAtTime) {
      ...
      if( this.bufferSource && undefined !== this.bufferSource.noteOff){
        this.bufferSource.stop = this.bufferSource.noteOff
      }

      this.bufferSource.stop(stopAtTime);
    };

    // stop()
    Audia.prototype.stop = function (stopAtTime) {
      if (stopAtTime === undefined) {
        stopAtTime = 0;
      }
      ...
      this.pause(stopAtTime);
      this.currentTime = 0;
    };

    // addEventListener()
    Audia.prototype.addEventListener = function (eventName, callback) {
    ...
    // removeEventListener()
    Audia.prototype.removeEventListener = function (eventName, callback) {
